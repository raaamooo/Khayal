...
export default function WebsitePricing({ currency }: { currency: 'EGP' | 'USD' }) {
  ...
  const website = content.pricing?.website || {};
  const noCode = website.noCode || {};
  ...
  const codedTiers = allTiers.filter(tier => !tier.isNoCode) || [
    {
      name: 'Starter',
      pages: '1–3 pages',
        ...
        <div className="bg-surface rounded-xl p-6">
          <h3 className="font-syne text-[24px] font-bold mb-4 flex items-center gap-2">
            <span className="px-3 py-1 rounded-full bg-surface/20 text-fog text-xs">
              {noCode.title || 'No-Code'}
            </span>
            {noCode.subtitle || 'Webflow / Framer'}
          </h3>
